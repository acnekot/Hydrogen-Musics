...
export const initSettings = () => {
    windowApi.getSettings().then(settings => {
        ...
        if (settings.customBackground) {
            ...
            // 与已有（本地持久化）值合并，避免缺省覆盖用户配置
            customBackgroundEnabled.value = cb.enabled ?? customBackgroundEnabled.value
            customBackgroundImage.value = cb.image ?? customBackgroundImage.value
            customBackgroundMode.value = cb.mode ?? customBackgroundMode.value
            customBackgroundBlur.value = cb.blur ?? customBackgroundBlur.value
            customBackgroundBrightness.value = cb.brightness ?? customBackgroundBrightness.value
            customBackgroundApplyToChrome.value = cb.applyToChrome ?? customBackgroundApplyToChrome.value
            customBackgroundApplyToPlayer.value = cb.applyToPlayer ?? customBackgroundApplyToPlayer.value
        }
