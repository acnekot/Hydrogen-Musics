...
module.exports = IpcMainEvent = (win, app, lyricFunctions = {}) => {
    ...
    ipcMain.on('set-settings', (e, settings) => {
        const incoming = ensureCustomBackgroundDefaults(JSON.parse(settings))
        // 同步一份兼容字段，便于旧代码读取
        if (incoming.customBackground) {
            const cb = incoming.customBackground
            incoming.customBackgroundEnabled = cb.enabled
            incoming.customBackgroundImage = cb.image
            incoming.customBackgroundMode = cb.mode
            incoming.customBackgroundBlur = cb.blur
            incoming.customBackgroundBrightness = cb.brightness
            incoming.customBackgroundApplyToChrome = cb.applyToChrome
            incoming.customBackgroundApplyToPlayer = cb.applyToPlayer
        }
        settingsStore.set('settings', incoming)
        registerShortcuts(win)
    })
    const ensureCustomBackgroundDefaults = (settings) => {
        const defaults = {
            enabled: false,
            image: '',
            mode: 'cover',
        ...
        if (!target.music) target.music = {}
        if (!target.local) target.local = {}
        if (!target.shortcuts) target.shortcuts = []
        if (!target.other) target.other = {}
        if (!target.customBackground) target.customBackground = {}
        // 兼容旧版本平铺字段
        const legacy = {
            enabled: target.customBackgroundEnabled,
            image: target.customBackgroundImage,
            mode: target.customBackgroundMode,
            blur: target.customBackgroundBlur,
            brightness: target.customBackgroundBrightness,
            applyToChrome: target.customBackgroundApplyToChrome,
            applyToPlayer: target.customBackgroundApplyToPlayer,
        }
        Object.keys(defaults).forEach(key => {
            if (typeof target.customBackground[key] === 'undefined') {
                const legacyVal = legacy[key]
                target.customBackground[key] = typeof legacyVal !== 'undefined' ? legacyVal : defaults[key]
            }
        })
        // 同步一份兼容字段，便于旧代码读取
        target.customBackgroundEnabled = target.customBackground.enabled
        target.customBackgroundImage = target.customBackground.image
        target.customBackgroundMode = target.customBackground.mode
        target.customBackgroundBlur = target.customBackground.blur
        target.customBackgroundBrightness = target.customBackground.brightness
        target.customBackgroundApplyToChrome = target.customBackground.applyToChrome
        target.customBackgroundApplyToPlayer = target.customBackground.applyToPlayer
        return target
    }
    ...
    ipcMain.handle('get-settings', async () => {
        const settings = await settingsStore.get('settings')
        if (settings) {
            const hydrated = ensureCustomBackgroundDefaults(settings)
            settingsStore.set('settings', hydrated)
            return hydrated
        } else {
            let initSettings = {
                music: {
            ...
            initSettings = ensureCustomBackgroundDefaults(initSettings)
