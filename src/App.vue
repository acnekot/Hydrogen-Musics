...
let lastAppliedAppScale = null;

const applyAppScale = rawScale => {
    const numeric = Number(rawScale);
    const base = Number.isFinite(numeric) ? numeric : 1;

    const clamped = Math.min(2, Math.max(0.8, Math.round(base * 100) / 100));
    if (lastAppliedAppScale !== clamped) {
        lastAppliedAppScale = clamped;
        ...
        if (Math.abs(clamped - 1) < 0.001) {
            ...
            root.style.removeProperty('--app-scale-inverse');
        } else {
            root.style.zoom = String(clamped);
            root.style.setProperty('--app-scale', String(clamped));
            root.style.setProperty('--app-scale-inverse', String(Math.round((1 / clamped) * 100) / 100));
        }
    }
};
