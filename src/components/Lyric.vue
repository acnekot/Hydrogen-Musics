...
// 将实时频谱映射为平滑的 0~1 级别，模拟衰减与攻击（暂停/静音时快速回落）
const updateVisualizerLevels = (size, resolveTarget, { paused = false } = {}) => {
    ...
    const release = paused ? 0.6 : 0.1; // 暂停/静音时更快下降
    let peak = 0;
...
// 仅在确实没有可展示的原文歌词时才显示占位，避免切换时占满区域
const lyricPlaceholderVisible = computed(() => {
    return !hasLyricsList.value || lyricType.value.indexOf('original') === -1 || !hasAnyLyricContent.value;
});
...
// 当区域从隐藏 -> 显示时，确保可视化器尺寸同步（不再额外延长隐藏时间）
watch(
    lyricAreaVisible,
    async (visible) => {
...
const changeProgressLyc = async (time, index) => {
    ...
    await nextTick();
    syncLyricPosition();
    // 播放状态下再补一次同步，等待进度/索引回调完成
    if (playing.value) {
        requestAnimationFrame(() => syncLyricPosition());
        setTimeout(() => syncLyricPosition(), 250);
    }
};
...
<template>
    <div class="lyric-container" :class="lyricContainerClasses">
        <canvas
            ...
            <div v-show="hasLyricsList && hasAnyLyricContent && lyricShow && lyricType.indexOf('original') != -1" class="lyric-area" :class="{ 'no-flash': suppressLyricFlash }" ref="lyricScroll">
                ...
                <div class="lyric-line" :style="{ transform: 'translateY(' + lineOffset + 'Px)' }" v-for="(item, index) in lyricsObjArr" v-show="item.lyric" :key="index">
                    ...
                    <div
                        ...
                        :class="{ 'music-interlude-in': interludeAnimation, 'music-interlude-fast-close': interludeFastClose }"
                    >
